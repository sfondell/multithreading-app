		...
		try {
			while ((responseLine = input_stream.readLine()) != null) {
				if (responseLine.startsWith("#welcome")) {
					...
					System.out.println(responseLine.substring(9) + " has joined");
				}
					...
					System.out.println(responseLine.substring(7) + " has left");
				}
					...
					String user = responseLine.substring(10);
					System.out.println(user + " wants to be friends, reply '@friend " +
						user + "' to accept or '@deny " + user + "' to deny");
				}
				else if (responseLine.startsWith("#OKfriends")) {
					String[] friends = responseLine.split("\\s+");
					System.out.println(friends[1] + " and " + friends[2] + " are now friends");
				}
				else if (responseLine.startsWith("#FriendRequestDenied")) {
					System.out.println(responseLine.substring(21) + " has denied your friend request");
				}
				else if (responseLine.startsWith("#NotFriends")) {
					String[] unfriends = responseLine.split("\\s+");
					System.out.println(unfriends[1] + " and " + unfriends[2] + " are no longer friends");
				}
				else {
